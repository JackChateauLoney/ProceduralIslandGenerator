...
public class DelaunayTriangulation : MonoBehaviour
{
	...
	public static List<Triangle> TriangulateByFlippingEdges(List<Vector3> sites)
	{
		...
		List<HalfEdge> halfEdges = MeshOperations.TransformFromTriangleToHalfEdge(triangles);
		...
		while (true)
		{
			...
			for (int i = 0; i < halfEdges.Count; i++)
			{
				...
				if (MeshOperations.IsPointInsideOutsideOrOnCircle(aPos, bPos, cPos, dPos) < 0f)
				{
					//Are these the two triangles that share this edge forming a convex quadrilateral?
					//Otherwise the edge cant be flipped
					if (MeshOperations.IsQuadrilateralConvex(aPos, bPos, cPos, dPos))
					{
						...
						if (MeshOperations.IsPointInsideOutsideOrOnCircle(bPos, cPos, dPos, aPos) < 0f)
						{
							continue;
						}
