...
public class RegionBiome : MonoBehaviour
{
    ...
    [SerializeField] GameObject cropPrefab = null;
    ...
    void GenerateField()
    {
        ...
        for (int i = 0; i < Mathf.Min(1000, cropPoints.Count); i++)
        {
            ...
            if (Physics.Raycast(cropPoints[i], Vector3.down * 200, out hit, 200f))
            {
                ...
                GameObject newCrop = Instantiate(cropPrefab, transform);
    ...
    void GenerateTown()
    {
        ...
        roadLine.Add(Vector3.Lerp(roadPoints[roadPoints.Count - 2], roadPoints[roadPoints.Count - 3], 0.5f));
